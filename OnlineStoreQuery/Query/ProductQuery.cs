...
namespace OnlineStoreQuery.Query
{
    public class ProductQuery : IProductQuery
    {
        ...
        {
            var inventory = inventoryContext.Inventory.Select(x => new { x.ProductId, x.UnitPrice, x.InStock }).ToList();
        ...
        {
            var inventory = inventoryContext.Inventory.Select(x => new { x.ProductId, x.UnitPrice }).ToList();
            var discounts = discountContext.CustomerDiscounts
                .Where(x => x.StartDate < DateTime.Now && x.EndDate > DateTime.Now)
