...
namespace InventoryMangement.Infrastructure.EFCore.Repository
{
    public class InventoryRepository : RepositoryBase<long, Inventory>, IInventoryRepository
    {
        ...
        {
            return inventoryContext.Inventory.FirstOrDefault(x => x.ProductId == productId);
        }

        public EditInventory GetDetails(long id)
        {
            return inventoryContext.Inventory.Select(x => new EditInventory
            {
                Id = x.Id,
        ...
        {
            var accounts = accountContext.Accounts.Select(x => new {x.Id, x.Fullname}).ToList();
            var inventory = inventoryContext.Inventory.FirstOrDefault(x => x.Id == inventoryId);
        ...
        public List<InventoryViewModel> Search(InventorySearchModel searchModel)
        {
            ...
            var query = inventoryContext.Inventory.Select(x => new InventoryViewModel
            {
                Id = x.Id,
                UnitPrice = x.UnitPrice,
                InStock = x.InStock,
